...
describe('theme switching', () => {
  test('style and mode change independently, Cmd-Shift-D preserves the style', async () => {
    installNative(memoryNative({ '/w/a.md': 'a' }))
    const manager = new Manager(await SettingsStore.load(), await RootFolders.load(), '/home/dev')
    expect(manager.theme.id).toBe('paper')
    manager.setStyle('compact')
    expect(manager.theme.id).toBe('compact-light')
    expect(manager.toast.text).toBe('Compact Light appearance')
    manager.toggleLightDark()
    expect(manager.theme.id).toBe('compact-dark')
    manager.setStyle('default')
    expect(manager.theme.id).toBe('dark')
    manager.setThemeMode(false)
    expect(manager.theme.id).toBe('paper')
    expect(manager.settings.data.themeID).toBe('paper')
  })
})
...
describe('text sizes', () => {
  ...
  test('the chrome CSS carries the theme block and the derived caption size', async () => {
    ...
    const css = chromeCSS({ ...defaultSettings(), fontDefault: 18, themeID: 'compact-dark' })
    expect(css).toContain('--font-default: 18px')
    expect(css).toContain('--font-small: 16px')
    expect(css).toContain('--bg: #1E1C1A')
    expect(css).toContain('--document-font-offset: -2px')
