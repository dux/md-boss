...
describe('buildChildren', () => {
  test('every folder on the way to a document exists, nothing else does, each level sorted', () => {
    const { buildChildren } = require('../src/models/fileTree') as typeof import('../src/models/fileTree')
    const children = buildChildren('/r', ['/r/top.md', '/r/docs/deep.md', '/r/docs/inner/x.md', '/r/10.md', '/r/9.md', '/r/Zeta/a.md', '/elsewhere/no.md'])
    expect(children.get('/r')!.map((n) => n.name)).toEqual(['docs', 'Zeta', '9.md', '10.md', 'top.md'])
    expect(children.get('/r/docs')!.map((n) => n.name)).toEqual(['inner', 'deep.md'])
    expect(children.get('/r/docs/inner')!.map((n) => n.path)).toEqual(['/r/docs/inner/x.md'])
    expect(children.has('/elsewhere')).toBe(false)
    expect(children.get('/r')!.find((n) => n.name === 'docs')!.isDir).toBe(true)
  })

  test('a root with a trailing slash and duplicate paths are tolerated', () => {
    const { buildChildren } = require('../src/models/fileTree') as typeof import('../src/models/fileTree')
    const children = buildChildren('/r/', ['/r/a.md', '/r/a.md'])
    expect(children.get('/r')!.map((n) => n.path)).toEqual(['/r/a.md'])
  })
})
