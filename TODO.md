...
The working list. Phases in order; a phase is done when every box in it is ticked.
...
- [x] frontend carried over from the Tauri port; `native/tauri.ts` and the `@tauri-apps/*` deps dropped
...
## 6. Server ports (from the Tauri port's Rust commands)
...
- [ ] `hammer icon` from `icons/AppIcon.svg`
